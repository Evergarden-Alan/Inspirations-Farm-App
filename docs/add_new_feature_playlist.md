...
## 二十八、阶段 2 实施记录（2026-07-31）

已实现：

- 对 `version: 1`、时间戳、精确字段集合、规范化 URL、外部字符串 ID、封面域名、数量上限和重复 ID 做严格配置校验。
- 文件不存在时以合法空配置开始，并在首次添加时通过 GitHub Contents API 创建 `Areas/FocusPlaylists/playlists.json`。
- 新合集写入 `addedAt`/`metadataUpdatedAt`；相同合集元数据未变化时原样返回，不发起 PUT，不产生无意义 commit。
- 标题、封面、owner、canonical URL 或数量变化时保留原添加时间，只刷新元数据与配置更新时间。
- 删除合集通过同一份版本化配置写回；最后一个合集删除后保留合法空配置文件。
- GitHub 409 时重新读取最新 SHA 和内容后重做幂等判断；连续冲突映射为 `CONFIG_CONFLICT`。
- JSON 损坏、未知版本、非法字段、非 base64 或超限文件映射为 `CONFIG_CORRUPTED`，任何写入前即停止。
- GitHub 普通 HTTP 错误新增带状态码的安全错误类型，对现有调用仍保留脱敏消息。

验证结果：

- 覆盖配置往返、未知版本、额外字段、重复添加、元数据刷新、删除、404 创建、409 重读去重和损坏配置不覆盖。
- 应用全部单元测试、ESLint 和 TypeScript 检查通过。
...
## 三十、阶段 4 实施记录（2026-07-31）

已实现：

- 正式 `/settings` 页面与首页 `aria-label="打开设置"` 入口，使用 Next.js `Link` 做客户端导航。
- 设置页复用农场背景、品牌栏、主题切换、PIN 锁屏、鉴权过期处理和全局 Toast。
- 页面当前只包含 Playlist URL、解析并加入、已配置合集和删除，未加入默认值、权重、排序、开关或音质等额外配置。
- 新增/刷新后直接使用服务端返回记录更新列表，避免 GitHub 读副本延迟；重复合集显示信息提示而不是错误。
- 合集卡片显示封面、标题、UP 主、视频数量、来源和加入日期；不显示完整视频条目或任何播放入口。
- 删除采用明确的二次确认状态；成功后立即从列表移除。
- Bilibili 图片域名加入严格的 Next Image HTTPS remote pattern。

验证结果：

- ESLint、TypeScript、全部单元测试和 Next.js 生产构建通过，`/settings` 成功生成。
- 桌面 Chromium 1440x1100 解锁实测：空状态、两栏布局、主题样式和返回导航正常，页面宽度 1440、无横向溢出。
- 只读 API 返回真实空配置，视觉验证期间没有向 GitHub 添加或删除合集。

结论：阶段 4 完成，进入阶段 5。

---

## 三十一、阶段 5 实施记录（2026-07-31）

已实现：

- 独立 `inspirations-farm-focus` IndexedDB 与 `focus-playlists` object store，按规范化合集 ID 保存可重建条目缓存。
- 缓存逐字段校验 schemaVersion、BVID、可空 CID、sourceIndex、标题、时长、总数和重复索引；损坏内容按未命中处理。
- 24 小时缓存新鲜度判断；未来时间戳和过期缓存不会被误判为新鲜。
- 设置页成功解析合集后写入完整条目缓存，删除长期配置后同步删除相应缓存；IndexedDB 失败不回滚已成功的 GitHub 操作。
- `farm_focus_player` version 1 状态模型，保存 focusSessionId、当前 playlist/BVID/CID/sourceIndex、播放位置、历史游标、最多 20 条真实历史和更新时间。
- 上一首回到实际历史；从历史回退后下一首先沿已有历史前进，在历史分叉处追加新条目会丢弃旧的前向分支。
- 状态使用精确字段白名单，短效音频 URL、签名、标题等额外字段会使状态无效，确保中继 URL 不进入 localStorage。
- JSON 损坏、结构无效或 focusSessionId 不一致时安全清理旧状态。

验证结果：

- 覆盖缓存往返/过期/损坏、播放时间恢复、session 隔离、历史前后导航、分叉替换、20 条上限和签名 URL 拒绝。
- 全部单元测试、ESLint、TypeScript 和 Next.js 生产构建通过。

结论：阶段 5 完成，进入阶段 6。

---

## 三十二、阶段 6 实施记录（2026-07-31）

已实现：

- 新增挂载在 `DailyDashboard` 层的常驻 `FocusAudioController`；专注弹窗收起时音频元素继续存在，只有结束或中止专注才停止音频并清理播放状态。
- 启动时读取 GitHub 合集配置与 IndexedDB 条目缓存：新鲜或过期缓存均可立即用于播放，缺失和超过 24 小时的缓存按需刷新，刷新失败时保留旧缓存。
- 两阶段均匀随机先选合集、再选条目；优先排除最近 20 首和当前条目，候选不足时逐级放宽，并允许测试注入确定性随机源。
- 新条目先通过 `audio-ticket` 重新确认并补齐 CID，再写入播放历史；短效中继 URL 只赋给内存中的 `<audio>`，不会进入 localStorage。
- 上一首返回真实历史；历史回退后的下一首先沿前向历史继续，到达末尾后随机追加；自然播放结束自动执行下一首。
- 单条票据失效时有限尝试其他候选；媒体错误先重新申请当前条目票据，再有限随机切换，连续失败后停止自动切换并保留手动重试入口。
- 暂停专注同步暂停音频；继续专注只恢复此前正在播放的音频。手动暂停音频不影响计时。
- 每 10 秒及 `pagehide`、页面隐藏、切换音频、播放/暂停、收起弹窗时保存位置；刷新恢复同一 playlist/BVID/CID/sourceIndex 和位置，但等待用户手势继续播放。
- 专注弹窗无合集时只显示设置入口；有合集时只显示上一首、播放/暂停、换一首三枚图标按钮和低干扰状态，不渲染标题、封面、序号、总数、进度条、iframe 或原生音频控件。

验证结果：

- 新增随机策略测试，覆盖两阶段选择、近期防重复、候选耗尽放宽和非法随机源；全部 9 个测试文件通过。
- ESLint、TypeScript 和 Next.js 生产构建通过。
- 桌面 Chromium 1440x1100 验证空配置状态：设置入口正确、无媒体按钮、无原生控件和横向溢出。
- 通过浏览器网络拦截构造仅用于 UI 的内存合集验证三按钮状态：未泄露标题或中继 URL、无 iframe、上一首在无历史时禁用、换一首可用；收起弹窗后常驻音频元素与计时/播放恢复状态仍保留。
