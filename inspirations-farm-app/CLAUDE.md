...
Personal inspiration management system. GitHub repo (`Evergarden-Alan/Note`) as flat-file database (Obsidian vault). Next.js 16 App Router + React 19 + Tailwind v4 + shadcn/ui (base-nova).
...
- **Shell renders instantly** — header + skeleton are streamed immediately
- **Skeleton pulses** — `DashboardSkeleton` with `animate-pulse`, matching the two-column grid
- **Data fetched server-side** — bypasses API routes, calls GitHub directly via `GITHUB_PAT`
...
| `src/lib/rollover.ts` | Daily task rollover (migrate undone tasks to next day) |
...
Client components manage their own state after initial SSR seed.
